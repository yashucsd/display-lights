 ...
 * Sources of Help
 */ 
 	.global ripple         	 ! Declares the sumbol to be globally visible so
 ...
 * 						const int rippleCnt);
 * Description: This function rotates the current light patterns in the light
 * 				banks by rippleCnt places displaying the light banks after each
 * 				single bit rotate. If rippleCnt is positive, rotate left; if
 * 				rippleCnt is negative, rotate right. The rippleCnt may be any
 * 				valid int value. Call displayLights() after each single bit
 * 				rotate, giving the ripple effect.
 * Parameters:
 * 	arg 1: lightBank - the lights to be rotated
 *	arg 2: rippleCnt - the number of bits to rotate
 *
 * Side Effects: None.
...
Lloop:
	...
	mov 	%l0, %o0 			! sends first address of lightBank[] to operator
	call 	displayLights 		! runs displayLights w new lightBank
	nop
...
right:
	...
	neg 	%i1 				! the rotate count is negative
	and 	%i1, MASK, %i1 		! masks rotateCnt for lower 6 bits
